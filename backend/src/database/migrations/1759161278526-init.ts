...
import { Kysely, sql as sqlTag } from 'kysely';

async function sql(db: Kysely<any>, query: string) {
  return sqlTag.raw(query).execute(db);
}

export async function up(db: Kysely<any>): Promise<void> {
  await sql(db, `CREATE EXTENSION IF NOT EXISTS "uuid-ossp";`);
  await sql(db, `CREATE EXTENSION IF NOT EXISTS "pgcrypto";`);
  await sql(
    db,
    `
  ...
  await sql(
    ...
    CREATE INDEX idx_ledger_name ON ledgers(name);
    CREATE INDEX idx_ledger_deleted_at ON ledgers(deleted_at);
    CREATE INDEX idx_ledger_tigerbeetle_id ON ledgers(tiger_beetle_id);
    `,
  );
  await sql(
    db,
...
export async function down(db: Kysely<any>): Promise<void> {
  ...
  await sql(db, `DROP TABLE IF EXISTS ledger_account_metadata CASCADE;`);
  await sql(db, `DROP TABLE IF EXISTS ledger_accounts CASCADE;`);
  await sql(db, `DROP TABLE IF EXISTS currencies CASCADE;`);
  await sql(db, `DROP TABLE IF EXISTS ledger_metadata CASCADE;`);
  await sql(db, `DROP TABLE IF EXISTS ledgers CASCADE;`);
  await sql(db, `DROP TABLE IF EXISTS keys CASCADE;`);
  await sql(db, `DROP TABLE IF EXISTS users CASCADE;`);

  // Drop custom function
  await sql(db, `DROP FUNCTION IF EXISTS uuid_generate_v7() CASCADE;`);
}
